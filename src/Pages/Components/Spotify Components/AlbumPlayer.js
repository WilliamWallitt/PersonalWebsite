...
export default class AlbumPlayer extends React.Component{
    ...
    render() {
        ...
        if (!this.state.show_track) {
            return (
                <ListGroup.Item><code>{this.props.item}</code><strong><br/>{this.props.item2}</strong> {this.props.item1 ? " Created At: " + this.props.item1.slice(0, 7) : ""} <br/><br/> <Button variant={"outline-dark"} className="btn-sm" onClick={this.onClickHandler}>Play Song</Button> </ListGroup.Item>
            )
        }
